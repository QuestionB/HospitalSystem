...
public class hospital {
		 ...
		 static String host = "10.16.42.41";  //要连接的服务端IP地址  
	...
	public static String login(String name,String passWord,int tN) throws IOException {//医院客户端登陆 类型号3
	 ...
	 public static int addUser(String name,String passWord,int i,int tN,int xuanzhe) throws IOException {  //病人注册 类型号1
			
		    
	     Socket client = new Socket(host, port);  
	      //建立连接后就可以往服务端写数据了  
	     Writer writer = new OutputStreamWriter(client.getOutputStream());  
	      writer.write(name+","+passWord+","+i+","+tN+","+xuanzhe);  writer.flush();  
	      writer.write("eof\n");  
	      writer.flush();  
	      //写完以后进行读操作  
	     BufferedReader br = new BufferedReader(new InputStreamReader(client.getInputStream()));  
	 ...
	 public static int Medicine(String name,String pinyin,int numble,int tN,int xuanzhe,int price) throws IOException {  //病人注册 类型号1
			
		    
	     Socket client = new Socket(host, port);  
	      //建立连接后就可以往服务端写数据了  
	     Writer writer = new OutputStreamWriter(client.getOutputStream());  
	      writer.write(name+","+pinyin+","+numble+","+tN+","+xuanzhe+","+price);  writer.flush();  
	      writer.write("eof\n");  
	      writer.flush();  
	      //写完以后进行读操作  
	     BufferedReader br = new BufferedReader(new InputStreamReader(client.getInputStream()));  
